...
(function (dom) {
    ...
    dom.dloc = typeof document !== 'undefined' ? document.location : { hash: '', pathname: '', search: '' };
    dom.html5Base = null;
    ...
    function readLocation() {
        if (dom.html5Base == null) {
            ...
            // - IE returns '#'
            (dom.dloc.hash === '' || dom.dloc.hash === '#')
                ? '/'
                : dom.dloc.hash.substring(1);
            return hash;
        }
        else {
            return dom.dloc.pathname.substr(dom.html5Base.length);
        }
    }
...
function transformHtml5Path(path) {
    var _a = dom.dloc, hash = _a.hash, search = _a.search;
